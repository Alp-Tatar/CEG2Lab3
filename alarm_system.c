...
void system_update_state(alarm_system_t *system){ //Sarp Ekin Berktay and Alp Tatar
  ticks++;
	switch (system->state){
		int k;
    case UNARMED: 
			if (system->usr_logged_in != NULL) {  // to check if there is a user logged in
				 system->state = WAITING_TO_ARM;
				system->usr_logged_in = NULL;    // reset it back to null
				ticks =0;
			}
      break;
    case WAITING_TO_ARM: 
			if (ticks >= 500) {    //when its more than 500 ticks
				system->state = ARMED;
				system->usr_logged_in = NULL;
			}else if (system->usr_logged_in != NULL) {
				system->state = UNARMED;
				system->usr_logged_in = NULL;
			}
      break;
    case ARMED: 
			...
			for (k = 0; k < 64; ++k) {  //for loop to check if any of the sensors are activated
				if (system->sensor_list[k].state == TRIGGERED) {
    ...
    case ALARMED: 
		 if (system->usr_logged_in != NULL) {
					if (system->usr_logged_in->is_super_user) {  //to check if a super user has logged in
							system->state = UNARMED;
							system->usr_logged_in = NULL;

							}
				}
			for (k = 0; k < 64; ++k) {    // reset all the sensors
					sensorm_reset(&system->sensor_list[k]);
			}
      break;
  }
